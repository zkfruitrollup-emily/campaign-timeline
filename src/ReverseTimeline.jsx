...
const PHASES = [
  { id: "kickoff", label: "Kickoff Call", owner: "client", days: 0 },
  { id: "concepts", label: "Influencer Concepts & Script Due", owner: "influencer", days: 3 },
  { id: "clientScript", label: "Client Script Feedback Due", owner: "client", days: 2 },
  { id: "reviseScript", label: "Influencer Revised Script Due", owner: "influencer", days: 2 },
  { id: "filming", label: "R1 Content Due", owner: "influencer", days: 5, slider: true },
  { id: "clientR1", label: "Client Review R1 Due", owner: "client", days: 2 },
  { id: "influencerR1Revisions", label: "Influencer R1 Revisions Due", owner: "influencer", days: 2 },
  { id: "clientR2", label: "Client Review R2 Due", owner: "client", days: 1 },
  { id: "influencerR2Revisions", label: "Influencer R2 Revisions Due", owner: "influencer", days: 1 },
  { id: "finalApproval", label: "Client Final Approval Due", owner: "client", days: 1 },
];

const C = {
  bg: "#ffffff", bgCard: "#f7f5f2", bgHighlight: "#f0ecf8", borderLight: "#eeebe6", border: "#e0dbd5",
...
function subtractBusinessDays(date, days) {
  let result = new Date(date), subtracted = 0;
  while (subtracted < days) { result.setDate(result.getDate() - 1); if (!isWeekend(result)) subtracted++; }
  return result;
}
function subtractAllDays(date, days) { let r = new Date(date); r.setDate(r.getDate() - days); return r; }
function nudgeBackward(date, preferWed = false) {
  let d = new Date(date);
...
function computeReverseTimeline(finalDate, phaseDays) {
  const reversed = [...PHASES].reverse();
  const results = [];
  let current = nudgeBackward(new Date(finalDate));
  for (let i = 0; i < reversed.length; i++) {
    const phase = reversed[i], days = phaseDays[phase.id] ?? phase.days;
    if (i === 0) { results.unshift({ ...phase, date: new Date(current), days }); continue; }
    let phaseDate = phase.owner === "influencer"
      ? nudgeBackward(subtractAllDays(current, days))
      : nudgeBackward(subtractBusinessDays(current, days), true);
    results.unshift({ ...phase, date: new Date(phaseDate), days });
    current = new Date(phaseDate);
  }
  return results;
}
...
export default function ReverseTimeline() {
  ...
  const deadlineDateObj = useMemo(() => { const [y,m,d] = deadline.split("-").map(Number); return new Date(y,m-1,d); }, [deadline]);
  const timeline = useMemo(() => computeReverseTimeline(deadlineDateObj, phaseDays), [deadlineDateObj, phaseDays]);
  const kickoffDate = timeline[0]?.date;
  ...
  return (
    <div style={{ fontFamily: "Georgia, serif", background: C.bg, minHeight: "100vh", color: C.text }}>
      <div style={{ borderBottom: `1px solid ${C.border}`, padding: "32px 40px 24px" }}>
        <div style={{ fontSize: "11px", letterSpacing: "0.2em", color: C.purple, marginBottom: "8px", textTransform: "uppercase" }}>Reverse Timeline Generator</div>
        <h1 style={{ fontSize: "28px", fontWeight: "400", margin: "0 0 4px", letterSpacing: "-0.02em" }}>Influencer Content Schedule</h1>
        <div style={{ fontSize: "13px", color: C.textMuted }}>Set your deadline · We calculate your kickoff</div>
      </div>
      ...
      <div style={{ padding: "32px 40px", maxWidth: "900px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "16px", marginBottom: "28px" }}>
          ...
          <div>
            <label style={{ fontSize: "10px", letterSpacing: "0.15em", color: C.purple, textTransform: "uppercase", display: "block", marginBottom: "8px", fontWeight: "600" }}>Final Approval Deadline ↓</label>
            <input type="date" value={deadline} onChange={e => setDeadline(e.target.value)}
              style={{ width: "100%", background: C.purpleLight, border: `1.5px solid ${C.purple}`, color: C.text, padding: "10px 12px", fontSize: "14px", fontFamily: "inherit", borderRadius: "2px", outline: "none", boxSizing: "border-box", colorScheme: "light" }} />
          </div>
          <div>
        ...
        {kickoffDate && (
          <div style={{ background: C.bgHighlight, border: `1.5px solid ${C.purple}`, borderRadius: "4px", padding: "16px 24px", marginBottom: "28px", display: "flex", alignItems: "center", gap: "16px" }}>
            <div style={{ fontSize: "10px", letterSpacing: "0.15em", color: C.purple, textTransform: "uppercase", whiteSpace: "nowrap" }}>Kickoff Must Start By</div>
            <div style={{ fontSize: "22px", fontFamily: "monospace", fontWeight: "700", color: C.purple }}>{formatDate(kickoffDate)}</div>
            <div style={{ fontSize: "12px", color: C.textMuted, marginLeft: "auto" }}>{totalDays} calendar days to deadline</div>
          </div>
        )}
        ...
        <div style={{ background: C.bgCard, border: `1px solid ${C.border}`, borderRadius: "4px", padding: "24px", marginBottom: "20px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            {timeline.map((phase, i) => {
              ...
              const isFinal = phase.id === "finalApproval";
              const ownerColor = isFinal ? C.purple : isInfluencer ? C.purple : C.amber;
              return (
                <div key={phase.id} style={{ display: "flex", alignItems: "center", gap: "16px", padding: "12px 0", borderBottom: i < timeline.length-1 ? `1px solid ${C.borderLight}` : "none", background: isFinal ? C.bgHighlight : "transparent" }}>
                  <div style={{ fontSize: "10px", color: C.textMuted, fontFamily: "monospace", minWidth: "16px", textAlign: "right" }}>{String(i+1).padStart(2,"0")}</div>
                  <div style={{ fontFamily: "monospace", fontSize: "13px", color: isFinal?C.purple:C.text, minWidth: "90px", fontWeight: "700" }}>{formatDate(phase.date)}</div>
                  <div style={{ color: C.border, fontSize: "12px" }}>—</div>
                  <div style={{ fontSize: "14px", color: isFinal?C.purple:C.textMid, flex: 1, fontWeight: isFinal?"600":"400" }}>{phase.label}</div>
                  <div style={{ fontSize: "9px", letterSpacing: "0.12em", textTransform: "uppercase", color: ownerColor, background: isFinal?C.purpleLight:isInfluencer?C.purpleLight:C.amberLight, padding: "3px 8px", borderRadius: "2px", whiteSpace: "nowrap", fontWeight: "600" }}>
                    {phase.id === "kickoff" ? "both" : isFinal ? "deadline" : isInfluencer ? "talent" : "client"}
                  </div>
                </div>
              );
            })}
